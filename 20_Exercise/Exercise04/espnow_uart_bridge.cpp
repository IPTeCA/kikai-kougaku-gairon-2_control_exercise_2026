...
static Stream* g_relay_port = nullptr;
static int g_led_pin = 2;
static bool g_echo_received_to_command = true;
static bool g_relay_received_to_relay = false;

void configure(const uint8_t peerMac[6], Stream& commandPort, int ledPin, Stream* relayUart) {
  g_peer_mac = peerMac;
  g_uart_port = &commandPort;
  g_relay_port = relayUart;
  g_led_pin = ledPin;
  g_tx_seq = 0;
  g_bad_frames = 0;
  g_sent_lines = 0;
  g_recv_lines = 0;
  g_echo_received_to_command = true;
  g_relay_received_to_relay = false;
}

void setEchoReceivedToCommandPort(bool enabled) {
  g_echo_received_to_command = enabled;
}

void setRelayReceivedToRelayPort(bool enabled) {
  g_relay_received_to_relay = enabled;
}
...
bool handleLocalCommand(const char* line) {
  ...
  if (strcmp(line, "/help") == 0) {
    g_uart_port->println("[INFO] USB: /stat, /mac, /help — other lines go to ESP-NOW (servo cmds, etc.)");
    if (g_relay_port) {
      g_uart_port->println("[INFO] Serial1: IMU lines from nRF are forwarded automatically (no slash cmds)");
    }
    return true;
  }
  return false;
...
void handleReceivedLine(const uint8_t* line, uint16_t len) {
  if (g_echo_received_to_command) {
    writeReceivedLine(line, len);
  }
  if (g_relay_received_to_relay && g_relay_port && g_relay_port != g_uart_port) {
    g_relay_port->write(line, len);
    g_relay_port->write('\n');
  }
}
...
static void processStreamLines(Stream* port, char* line, uint16_t* idx, bool allowLocalCommand) {
  if (!port) return;
  ...
  while (port->available()) {
    ...
    if (c == '\n') {
      if (*idx > 0) {
        line[*idx] = '\0';
        bool handled = false;
        if (allowLocalCommand) {
          handled = handleLocalCommand(line);
        }
        if (!handled) {
    ...
    if (*idx < kLineMax) {
      ...
      *idx = 0;
      port->println("[ERR] uart line too long");
    }
  }
}

void processUartLoop() {
  static char line_cmd[kLineMax + 1];
  static uint16_t idx_cmd = 0;
  static char line_relay[kLineMax + 1];
  static uint16_t idx_relay = 0;

  processStreamLines(g_uart_port, line_cmd, &idx_cmd, true);
  processStreamLines(g_relay_port, line_relay, &idx_relay, false);
}
